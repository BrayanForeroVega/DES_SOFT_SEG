 const db = require('../config/config');
   ...
   const User = {};
   User.findAll = (result) => {
     const sql = `SELECT id, Producto, descripcion, , valor, image, created_at, updated_at FROM Productos`;
     db.query(sql, (err, users) => {
       if (err) {
         console.log('Error al listar productos: ', err);
         result(err, null);
       } else {
         console.log('Productos encontrados: ', users.length);
         result(null, users);
       }
     });
    };
   User.findById = (id, result) => {
     const sql = `SELECT id, Producto, Valor, lastname, image, password FROM users WHERE id = ?`;
     db.query(sql,
       [id], (err, user) => {
         if (err) {
